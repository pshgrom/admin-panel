<template>
    <q-dialog v-model="showDialog" v-if="this.obj">
        <q-layout view="Lhh lpR fff" container class="bg-white dialog-layout"
                  style="min-width: 1000px;width: 1000px;max-height: 350px">
            <q-header bordered>
                <q-toolbar>
            ...
            <q-footer bordered>
                ...
                <custom-button v-if="!readonly" title="Сохранить" type="purple" @click="save" />
            </q-footer>
                ...
                <q-page padding>
                    <q-form ref="form" @submit="save" class="user-form" dense>
                        <div class="row form-row">
                            <div class="col-2">
                                <q-field outlined label="ЭДО ID" stack-label dense>
                                    <template v-slot:control>
                                        <div class="self-center full-width no-outline" tabindex="0">{{ obj.edo_id }}</div>
                                    </template>
                                </q-field>
                            </div>
                            <div class="col-10">
                                <q-field outlined label="Название организации в ЭДО" stack-label dense>
                                    <template v-slot:control>
                                        <div class="self-center full-width no-outline" tabindex="0">{{ obj.name }}</div>
                                    </template>
                                </q-field>
                            </div>
                        </div>
...
export default defineComponent({
    ...
    methods: {
        ...
        save(stay) {
            ...
            EdoApi.corrSave(this.obj).then((data) => {
                if (data._errors) {
